...
Haskell.createWebSocket = function (receive) {
  var that = {};
  var url = "ws://127.0.0.1:8023/websocket/";
  //var timerID = 0;
  var optReloadOnDisconnect = true;
  var ws = null;
  ...
  that.setReloadOnDisconnect = function (b) {
    optReloadOnDisconnect = b;
  };
...
Threepenny.createHTTP = function (receive) {
  ...
  var waitForEvents = function () {
    console.log("Pollingâ€¦ (%d signals so far)",signal_count);
    ...
    var req = $.ajax({
      ...
      success: function (events) {
        ...
        console.log("Running event" +(events.length>1?'s':'') +"â€¦")
        if (events.length) {
...
Haskell.initFFI = async function () {
  /////////////////////////////////////////////////////////////////////
  ...
  var receive = function (msg) {
    switch (msg.tag) {
      ...
      case "Debug":
        console.log("Server debug: %o", msg.contents);
        reply();
        break;

      case "Timestamp":
        console.log("Timestamp: %f ms", Haskell.performance.now());
        console.log(
    ...
    var that = function () {
      ...
      for (var i = 0; i < theargs.length; i++) {
        for (var j = 0; j < theargs[i].length; j++) {
          theargs[i][j] = theargs[i][j] >= 0 ? Math.round(theargs[i][j]) : 0;
        }
      }
...
Haskell.on = function (el, eventType, fun) {
  if (eventType === "livechange") {
    ...
    eventType.match("dragstart|dragenter|dragover|dragleave|drag|drop|dragend")
  ) {
...
$.fn.livechange = function (ms, trigger) {
  $(this).each(function () {
    ...
    var check = function () {
      var val = el.val();
      if (val != last_val) trigger.call(self);
      last_val = val;
    };
    var checker;
